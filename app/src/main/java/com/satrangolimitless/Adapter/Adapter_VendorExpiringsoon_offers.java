...
public class Adapter_VendorExpiringsoon_offers extends RecyclerView.Adapter<Adapter_VendorExpiringsoon_offers.ViewHolder> {
    Session session1;
    private final List<Offers_expiringsoon_model> category_home_models;
    private final Context context;


    public Adapter_VendorExpiringsoon_offers(ArrayList<Offers_expiringsoon_model> category_home_models, Context context) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_offers_expiring_soon, parent, false);
        return new ViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        final Offers_expiringsoon_model allCommunityModel = category_home_models.get(position);
...
//        holder.txtrowtext.setText(category_home_models.get(position).getMessage());
        ...
        holder.imagess.setOnClickListener(new View.OnClickListener() {
            @Override

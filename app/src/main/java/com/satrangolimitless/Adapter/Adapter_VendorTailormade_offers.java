...
public class Adapter_VendorTailormade_offers extends RecyclerView.Adapter<Adapter_VendorTailormade_offers.ViewHolder> {
    Session session1;
    private final List<Tailormadeoffers_model> category_home_models;
    private final Context context;


    public Adapter_VendorTailormade_offers(ArrayList<Tailormadeoffers_model> category_home_models, Context context) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_offers_tailormade, parent, false);
        return new ViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        final Tailormadeoffers_model allCommunityModel = category_home_models.get(position);

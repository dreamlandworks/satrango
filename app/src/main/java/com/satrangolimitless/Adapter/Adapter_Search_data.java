...
public class Adapter_Search_data extends RecyclerView.Adapter<Adapter_Search_data.ViewHolder> {
    Session session;
    private final List<Search_Data_Model> search_data_models;
    private final Context context;


    public Adapter_Search_data(List<Search_Data_Model> search_data_models, Context context) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        final Search_Data_Model allCommunityModel = search_data_models.get(position);


        session = new Session(context);

//        Glide.with(context).load(mediimage_url+allCommunityModel.getImage()). into(holder.img_user);
